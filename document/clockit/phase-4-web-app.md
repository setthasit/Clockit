...
- [x] 8.3: Manual flow against seeded backend: sign in → onboarding with map pin → employees (add, set rate, remove) → calendar shows seeded week incl. midnight-spanner as two segments + unverified dashed bar → table: set tip 100.00 on a 3-member day, shares sum exactly to 100.00 → CSV downloads and opens.
...
- **8.3 pass.** Sign-in → the guard's zero-employer redirect landed on `/onboarding`; the real map rendered and typing coordinates recentred it (`ll=20,0&z=2` → `ll=49.2827,-123.1207&z=15`); create → `/employees` with the "Add your first employee" empty state. Employees: invite showed `—` / `Invited` / `Set rate`, a second invite of the same address surfaced `already a member of this employer` inline in the dialog, the rate edit sent exactly one `PATCH {"hourly_rate_cents":1955}` (19.55 — the float trap, one request, no Enter-then-blur double write), and Remove collapsed the row into a read-only "Removed" section offering Re-invite. Calendar: the midnight spanner rendered as **two** segments measured at 22:00→24:00 (96 px) and 00:00→06:30 (312 px), summing to the seeded 510 min; the unverified shift had `border-style: dashed`; the `speed_anomaly` shift carried the flag icon; the three overlapping day shifts split to 40 px lanes while the spanner kept the full 119 px column, confirming per-cluster lanes. The popover's "View in table" link opened `/table?from=2026-08-08&to=2026-08-08` and the table honoured it, carrying the unverified marker across the entries join. **Tip $100.00 on a 3-member day split to $31.92 / $36.17 / $31.91 = exactly $100.00**, matching `SplitByMinutes`' largest-remainder tiebreak. CSV downloaded as `clockit-report-2026-08-09-2026-08-15.csv`, parsed clean under Python's strict `csv` (12 records, 9 fields each, BOM present), and every money column summed to the truth: `SUM(base_pay)` 1905.00 matching the on-screen range total, `SUM(tip_share)` 300.00 = the three tips entered.
- **8.4 pass.** Second employer created from the switcher's "New employer" (with the Cancel button that correctly does *not* appear during first-run onboarding), on `Asia/Tokyo` to make scoping unambiguous. It showed its own empty employee list; switching back restored Alice/Bob/Carol with their rates, and `localStorage.clockit.employerId` held the active id.
- **8.5 partial pass.** The "no employer yet" half was verified on the real first sign-in, which forced `/onboarding`. The 404 half was spot-checked with the app's own bearer token against both foreign-owned employers, a malformed id and a well-formed unknown id — **all four returned `404 NOT_FOUND`**, indistinguishable from each other, with the owned control returning 200. Not verified: a *second* Auth0 identity that is a member of someone else's employer while owning none; that path runs the same `GET /v1/employers` → `[]` code, but was deliberately skipped rather than tested.

**Defects the live run exposed** (none were findable with fixture data):

1. **Deep links were lost on every sign-in — fixed, `404eaa9` + `512353d`.** Task 2.1's `appState.returnTo` never worked: the SDK's `defaultOnRedirectCallback` uses `history.replaceState`, which does not notify react-router, so the router still matched the index route and its `<Navigate to="/calendar" replace/>` overwrote the destination a tick later. Reproduced twice (`/employees` and `/table?from=…&to=…` both landed on `/calendar`). Fixed with an `onRedirectCallback` that navigates through the exported data router. This also silently stripped the date scope from the calendar popover's "View in table" link for anyone arriving unauthenticated. The task-2.1 review predicted the mechanism and judged it harmless — it was not.
2. **Auth0 errors were swallowed silently — fixed, `f0a4279` + `2014394`.** A tenant misconfiguration returns `?error=…` to the origin; the SDK set `useAuth0().error` and nothing rendered it, so sign-in bounced back to `/sign-in` with no message and a button that looked simply broken. Every first-run tenant problem (missing client grant, wrong audience, `offline_access` off) presented identically. Now surfaced in a `Banner`.
3. **Backend: `users.email` is a unique index with no partial filter — NOT fixed, out of this phase's scope.** Any two users lacking an email collide on `""`, and the caller is told `409 EMAIL_TAKEN "email already registered to another account"` about an address they do not have. Hit as a real user during this run. Reachable in production whenever the custom email claim is absent for two identities. Wants a partial index (`{email: {$exists: true, $ne: ""}}`) or rejection of empty emails at the boundary — belongs to a backend phase.
4. **Tenant config, not code:** the Auth0 Action from design §3 that injects `https://clockit/email` and `https://clockit/email_verified` is **not deployed**, so every human login is stored with an empty email. The invite-claim path (`invited` → `active` on first login, matched by email) cannot work until it exists — which is also why 8.5's employee-only identity was not worth creating yet.

**Environment notes for the next run.** `cacheLocation: "memory"` means every full page load signs the operator out to `/sign-in`; the click re-auths silently through the SSO cookie, but this tenant additionally re-prompts consent each time because the API has "Allow Skipping User Consent" off. The SPA needs **User-delegated access** on the API (not Client access, which is M2M and cannot work for a public client). The seed's default owner email (`owner@acme.test`) collides with any Auth0 account signed up under the same address — pass `-owner-sub` with the real sub instead.
