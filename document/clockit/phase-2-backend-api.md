...
- 2.1 (fixed post-phase-3 QA): `users.email` is now **unique + partial** (`partialFilterExpression: {email: {$gt: ""}}`). It was plain-unique, so the first identity stored with `email: ""` — an M2M token, or any login before the Auth0 Action ships — claimed the value and every later one got `409 EMAIL_TAKEN` for an address it does not have, permanently. `$gt: ""`, not the obvious `$ne: ""`: MongoDB rejects `$ne` in a partial filter (`CannotCreateIndex … Expression not supported in partial index: $not`); the accepted set is equality/`$eq`, `$exists: true`, `$gt/$gte/$lt/$lte`, `$type`, `$and/$or/$in`, `$geoWithin/$geoIntersects`. **`EnsureIndexes` cannot migrate this** — it is idempotent only for identical specs, so a database carrying the old `email_1` fails startup with `IndexKeySpecsConflict`. Drop it once (`db.users.dropIndex("email_1")`) or drop the database; no migration code, since nothing is deployed.
