...
Before any work: read `docs/design.md` (authoritative design) and the relevant phase plan in `document/clockit/`. The plan's "Global conventions" section in `document/clockit/README.md` applies to every change.
...
- Prefer editing existing files; smallest diff that satisfies the plan task. Mark deliberate shortcuts with a `ponytail:` comment naming the ceiling and upgrade path.
...
Every phase plan ends with a Verification task — it is part of the work, not optional. Backend changes need `make test && make lint` green. If a library API differs from a plan snippet, follow the library's current docs and record the deviation in the phase completion notes.
